...
#            print('labeluni', np.unique(label_clf_t1.data.cpu().numpy()), np.unique(label_clf_t1.data.cpu().numpy()))
#            print(np.unique(label_cd.data.cpu().numpy()))
    ...
    def validation(self):
        ...
        for itera, data in enumerate(val_data_loader):
            ...
            label_clf_t1 = label_clf_t1.cuda().long()
            label_clf_t2 = label_clf_t2.cuda().long()
            #print('labeluni', np.unique(label_clf_t1.data.cpu().numpy()), np.unique(label_clf_t1.data.cpu().numpy()), np.unique(label_cd.data.cpu().numpy()))            
...
def main():
    ...
    parser.add_argument('--learning_rate', type=float, default=1e-6*5)
